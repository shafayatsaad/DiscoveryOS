...
const mlCards = [
  { label: "Evidence Ranker", value: "0.89", detail: "Weighted relevance and confidence score", icon: BarChart3 },
  { label: "Graph Centrality", value: "High", detail: "Active hypothesis cluster is highly connected", icon: GitBranch },
  { label: "Novelty Signal", value: "82%", detail: "Semantically distant from retrieved prior work", icon: BrainCircuit },
  { label: "Contradiction Load", value: "12", detail: "Conflicting claim clusters retained for review", icon: Activity },
];
        ...
        <div className="mx-auto flex w-full max-w-container-max flex-1 flex-col gap-6 px-5 py-6 sm:px-8 md:px-10 md:py-10">
          <Reveal>
            <header className="glass-panel rounded-xl p-5 sm:p-6">
              ...
              <p className="mt-3 max-w-3xl text-base leading-[1.6] text-on-surface-variant">
                Interpretable model signals for {project.title}. These mock outputs are displayed as signals, not scientific conclusions.
              </p>
            </header>
          </Reveal>
              ...
              return (
                <Reveal key={card.label} delay={index * 0.04}>
                  <article className="glass-panel rounded-xl p-5 transition-colors hover:bg-white/[0.04]">
                    ...
                    <p className="mt-2 font-display text-4xl font-semibold text-on-surface">{card.value}</p>
                    <p className="mt-3 text-sm leading-[1.55] text-on-surface-variant">{card.detail}</p>
                  </article>
                </Reveal>
              );
            })}
          ...
          <Reveal delay={0.12}>
            <section className="glass-panel rounded-xl p-5">
              <h2 className="mb-4 font-display text-2xl font-semibold text-on-surface">Feature Summary</h2>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                {["Evidence strength", "Semantic distance", "Graph support"].map((feature, index) => (
                  <div key={feature} className="rounded-lg border border-white/[0.05] bg-surface/50 p-4">
                    ...
                    <div className="h-1.5 overflow-hidden rounded-full bg-surface-container-highest">
                      <div className="h-full rounded-full bg-primary" style={{ width: `${86 - index * 7}%` }} />
                    </div>
                  </div>
                ))}
              </div>
